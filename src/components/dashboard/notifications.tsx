  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {!!notifications.workshop_new.length && (
            <div className="w-2 h-2 absolute top-2 right-2 bg-red-500 rounded-full"></div>
          )}
      ...
      <PopoverContent align="end" className="w-[250px] md:w-[350px] p-0">
        ...
        <ScrollArea className="w-full h-[400px] md:h-[500px]">
          {!!notifications.workshop_new.length ||
          !!notifications.workshop_readed.length ? (
            <div className="divide-y divide-secondary-lighter">
              {notifications.workshop_new.map((notification) => (
                <>
                  {notification.redirect_path ? (
                    <Link
                      href={notification.redirect_path}
                      key={notification.id}
                      className="space-y-1 p-4 block"
                    >
                      <h3 className="text-sm md:text-base font-semibold">
                        {notification.title.includes("%NUMBER_VALUE%")
                          ? notification.title.replace(
                              "%NUMBER_VALUE%",
                              notification.number_value!.toString()
                            )
                          : notification.title}
                      </h3>
              ...
              {notifications.workshop_readed.map((notification) => (
                <div
                  key={notification.id}
                  className="space-y-1 p-4 bg-neutral-200"
                >
            ...
            <p className="text-sm md:text-base m-4">
              No notification to see right now.
            </p>
          )}
        </ScrollArea>
        <Button
